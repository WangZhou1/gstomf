...
namespace omf {
	...
	public:
		...
		/**
		 * Check the Message reveiver is register.
		 * @return true/false
		 */
		virtual bool ReceiverCheck(unsigned)const = 0;
	public:///send
		/**
		 ...
		 * @return true/false
		 */
		virtual bool Send(unsigned target, const void *data, int size, unsigned flags = 0,const char* name=" ")=0;
		 ...
		 * @param size [in]:the msg body size
		 * @param flags [in]:the msg flags
		 * @param onDrop [in]:callback function is called when dropping this message.
		 * @param timeoutInMs [int]:the longest time to wait for receiving this message.
		 * @return true/false
		 */
		virtual bool Send(unsigned target,const void*data,int size,void(*onDrop)(const void* data, int size),int timeoutInMs,unsigned flags=0,const char* name=" ")=0;
		/**
		 * send msg use sender
		 ...
		 * @return true/false
		 */
		virtual bool Send(unsigned sender,unsigned target, const void *data, int size, unsigned flags = 0,const char* name=" ")=0;
		 ...
		 * @return true/false
		 */
		virtual bool Send(unsigned sender,unsigned target,const void*data, int size, void(*onDrop)(const void* data, int size), int timeoutInMs, unsigned flags=0, const char* name=" ")=0;
	public:
		/**
