 ...
 * @param data [in]:msg body
 * @param size [in]:msg size
 * @param onDrop [in]:callback function which is called when dropping this message.
 * @param timeoutInMs [in]:the longest time to wait for receiving this message.
 * @return true/false
 */
EXTERNC int omfMsgSiteUserSend1(const char* name, const void *data, int size, void(*onDrop)(const void* data, int size), int timeoutInMs);
...
/**
 * check the receiver is register.
 * @param name [in]: msg name.
 * @return true/fasle
 */
EXTERNC int omfMsgSiteUserReveiverCheck(const char* name);
