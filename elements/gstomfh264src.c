...
static gboolean gst_omf_h264_src_setcaps (GstBaseSrc * bsrc, GstCaps * caps);
static GstCaps *gst_omf_h264_src_src_fixate (GstBaseSrc * bsrc,
    GstCaps * caps);
...
gst_omf_h264_src_class_init (GstOmfH264SrcClass * klass)
{
  ...
  gstbase_src_class->set_caps = GST_DEBUG_FUNCPTR (gst_omf_h264_src_setcaps);
  gstbase_src_class->fixate = GST_DEBUG_FUNCPTR (gst_omf_h264_src_src_fixate);
...
gst_omf_h264_src_init (GstOmfH264Src * omfh264src)
{
  ...
  /* we operate in time */
  gst_base_src_set_format (GST_BASE_SRC (omfh264src), DEFAULT_FORMAT);
  gst_base_src_set_live (GST_BASE_SRC (omfh264src), FALSE);
  ...
  omfh264src->spts_ns = -1;
  omfh264src->lpts_ns = -1;
...
static GstCaps *
gst_omf_h264_src_src_fixate (GstBaseSrc * bsrc, GstCaps * caps)
{
  GstOmfH264Src *src = GST_OMF_H264_SRC (bsrc);
...
static gboolean
gst_omf_h264_src_setcaps (GstBaseSrc * bsrc, GstCaps * caps)
{
  const GstStructure *structure;
  GstOmfH264Src *omfh264src;
  guint i;
  guint n_lines;
  gint offset;

  omfh264src = GST_OMF_H264_SRC (bsrc);

  structure = gst_caps_get_structure (caps, 0);

  GST_OBJECT_LOCK (omfh264src);


  GST_OBJECT_UNLOCK (omfh264src);
...
gst_omf_h264_src_start (GstBaseSrc * basesrc)
{
  ...
  OmfH264SrcGetMediaInfo(src->omf_hd, &src->media);
