 ...
 */
/**
 * SECTION:element-omfh265src
 * @title: omfh265src
 * @see_also: #GstPcmSink
 *
 * The h265src element is a h265 source element.
 *
 * ## Example launch line
 * |[
 * gst-launch-1.0 -v omfh265src ! fakesink
 * ]| This pipeline will get hevc data to the pcmsink element.
 *
 */
...
#define GST_CAT_DEFAULT gst_omf_h265_src_debug

/* OmfH265Src signals and args */
enum
{
  /* FILL ME */
  SIGNAL_HANDOFF,
...
#define DEFAULT_GOPTYPE			OMF_H265_SRC_GOP_TYPE_IPPP
...
static GType
gst_omf_h265_src_get_gop_type (void)
{
  static GType omfh265src_gop_type_type = 0;
  static const GEnumValue omfh265src_gop_type[] = {
	{OMF_H265_SRC_GOP_TYPE_IBBP, "Gop type IBBP", "IBBP"},
	{OMF_H265_SRC_GOP_TYPE_IPPP, "Gop type IPPP", "IPPP"},
	{OMF_H265_SRC_GOP_TYPE_IIII, "Gop type IPPP", "IIII"},
	{0, NULL, NULL},
  };

  if (!omfh265src_gop_type_type) {
	omfh265src_gop_type_type =
		g_enum_register_static ("GstOmfH265SrcGopType", omfh265src_gop_type);
  }
  return omfh265src_gop_type_type;
}

#define _do_init \
    GST_DEBUG_CATEGORY_INIT (gst_omf_h265_src_debug, "omfh265src", 0, "omfh265src element");
#define gst_omf_h265_src_parent_class parent_class
G_DEFINE_TYPE_WITH_CODE (GstOmfH265Src, gst_omf_h265_src, GST_TYPE_BASE_SRC, _do_init);

static void gst_omf_h265_src_finalize (GObject * object);
static void gst_omf_h265_src_set_property (GObject * object, guint prop_id,
...
static gboolean gst_omf_h265_src_is_seekable (GstBaseSrc * basesrc);

static gboolean gst_omf_h265_src_event_handler (GstBaseSrc * src, GstEvent * event);
static void gst_omf_h265_src_get_times (GstBaseSrc * basesrc, GstBuffer * buffer,
    GstClockTime * start, GstClockTime * end);
static GstFlowReturn gst_omf_h265_src_create (GstBaseSrc * src, guint64 offset,
    guint length, GstBuffer ** buf);

static guint gst_omf_h265_src_signals[LAST_SIGNAL] = { 0 };

static GParamSpec *pspec_last_message = NULL;

static void
gst_omf_h265_src_class_init (GstOmfH265SrcClass * klass)
{
  ...
  gobject_class->finalize = gst_omf_h265_src_finalize;

  gobject_class->set_property = gst_omf_h265_src_set_property;
  gobject_class->get_property = gst_omf_h265_src_get_property;
  ...
  gstbase_src_class->event = GST_DEBUG_FUNCPTR (gst_omf_h265_src_event_handler);
  gstbase_src_class->get_times = GST_DEBUG_FUNCPTR (gst_omf_h265_src_get_times);
  gstbase_src_class->create = GST_DEBUG_FUNCPTR (gst_omf_h265_src_create);
}

static void
gst_omf_h265_src_init (GstOmfH265Src * omfh265src)
{
  omfh265src->silent = DEFAULT_SILENT;
  omfh265src->signal_handoffs = DEFAULT_SIGNAL_HANDOFFS;
  omfh265src->dump = DEFAULT_DUMP;
  omfh265src->format = DEFAULT_FORMAT;
  omfh265src->last_message = NULL;

  omfh265src->senid = DEFAULT_SENID;
  omfh265src->width = DEFAULT_WIDTH;
  omfh265src->height = DEFAULT_HEIGHT;
  omfh265src->framerate = DEFAULT_FRAMERATE;
  omfh265src->bitrate = DEFAULT_BITRATE;
  omfh265src->gop = DEFAULT_GOP;
  omfh265src->goptype = DEFAULT_GOPTYPE;
  omfh265src->codec = DEFAULT_CODEC ? strdup(DEFAULT_CODEC) : NULL;
  omfh265src->prerecidx = DEFAULT_PREREC_IDX;
  omfh265src->prerecpipe = DEFAULT_PREREC_PIPE ? strdup(DEFAULT_PREREC_PIPE) : NULL;
  omfh265src->shareidx = DEFAULT_SHARE;
  omfh265src->lowbw = DEFAULT_LOW_BW;
...
gst_omf_h265_src_finalize (GObject * object)
{
  ...
  src = GST_OMF_H265_SRC (object);

  g_free (src->last_message);
  
  if(src->omf_hd){
	 OmfH265SrcDestory(src->omf_hd);
  }
...
static gboolean
gst_omf_h265_src_event_handler (GstBaseSrc * basesrc, GstEvent * event)
{
  GstOmfH265Src *src;

  src = GST_OMF_H265_SRC (basesrc);
...
static void
gst_omf_h265_src_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstOmfH265Src *src;
  GstBaseSrc *basesrc;
  ...
  switch (prop_id) {
	...
	case PROP_GOP:
		...
		break;
	case PROP_GOPTYPE:
		src->goptype = (GstOmfH265SrcGopType)g_value_get_enum(value);
		break;
	case PROP_CODEC:
		g_free(src->codec);
...
static void
gst_omf_h265_src_get_property (GObject * object, guint prop_id, GValue * value,
    GParamSpec * pspec)
{
  GstOmfH265Src *src;
  GstBaseSrc *basesrc;

  g_return_if_fail (GST_IS_OMF_H265_SRC (object));
...
static GstBuffer *
gst_omf_h265_src_alloc_buffer (GstOmfH265Src * src, guint size, gpointer pdata, GDestroyNotify pfree)
{
  GstBuffer *buf;
  gpointer data;
...
static void
gst_omf_h265_src_get_times (GstBaseSrc * basesrc, GstBuffer * buffer,
    GstClockTime * start, GstClockTime * end)
{
  GstOmfH265Src *src;
...
gst_omf_h265_src_create (GstBaseSrc * basesrc, guint64 offset, guint length,
    GstBuffer ** ret)
{
  GstOmfH265Src *src;
  GstBuffer *buf;
  ...
  if(OmfH265SrcGetFrame(src->omf_hd, &frame)){
  	if(frame.data){
	  size = frame.size;
  ...
  if (src->signal_handoffs) {
    ...
    g_signal_emit (src, gst_omf_h265_src_signals[SIGNAL_HANDOFF], 0, buf,
        basesrc->srcpad);
...
static gboolean
gst_omf_h265_src_start (GstBaseSrc * basesrc)
{
  GstOmfH265Src *src;
  ...
  g_return_val_if_fail(OmfH265SrcSelectSensor(src->omf_hd, src->senid), FALSE);
  if(src->width){
  	g_return_val_if_fail(OmfH265SrcSetWidth(src->omf_hd, src->width), FALSE);
  }
  if(src->height){
  	g_return_val_if_fail(OmfH265SrcSetHeight(src->omf_hd, src->height), FALSE);
  }
  if(src->framerate){
  	g_return_val_if_fail(OmfH265SrcSetFrameRate(src->omf_hd, src->framerate), FALSE);
  }
  if(src->bitrate){
  	g_return_val_if_fail(OmfH265SrcSetBitRate(src->omf_hd, src->bitrate), FALSE);
  }
  if(src->gop){
   	g_return_val_if_fail(OmfH265SrcSetGop(src->omf_hd, src->gop), FALSE);
  }
  //if(src->goptype){
    g_return_val_if_fail(OmfH265SrcSetGopType(src->omf_hd, src->goptype), FALSE);
  //}
  if(src->prerecidx){
  	g_return_val_if_fail(OmfH265SrcSetPreRecord(src->omf_hd, src->prerecidx), FALSE);
  }		
  if(src->prerecpipe){
	g_return_val_if_fail(OmfH265SrcSetPreRecordPipe(src->omf_hd, src->prerecpipe), FALSE);	
  }
  if(src->cache){
	g_return_val_if_fail(OmfH265SrcSetCache(src->omf_hd, src->cache), FALSE);
  }
  if(src->lowbw){
...
static gboolean
gst_omf_h265_src_stop (GstBaseSrc * basesrc)
{
  GstOmfH265Src *src;
  ...
  if(src->omf_hd){
  	OmfH265SrcStatusDown(src->omf_hd, OMF_STATE_NULL);
  }
...
static gboolean
gst_omf_h265_src_is_seekable (GstBaseSrc * basesrc)
{
  GstOmfH265Src *src = GST_OMF_H265_SRC (basesrc);
